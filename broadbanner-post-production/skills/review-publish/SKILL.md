---
name: review-publish
description: "Push a generated review markdown into the member portal as an editable DRAFT via the BroadBanner MCP connector's create_review tool. Use when the user says 'publish the review', 'push it to the portal', 'create the review draft', or after episode-review completes in the post-production chain. Returns the app.broadbanner.com/app/reviews/<slug> URL for the member to review, edit, and publish. NO git, NO GitHub Pages — this replaces the old pages-publish flow. Production+ add-on (post_production_distribution)."
metadata:
...
the BroadBanner MCP connector's `create_review` tool, then return the portal URL so the
...
draft in `app.broadbanner.com/app/reviews/…`, and the member owns the publish action.
...
`create_review` tool fails closed as the backstop. When invoked by the `post-production`
...
| `reviewLabel`  | Rec.     | `Palantalk In Review`                               | From `effectiveReviewConfig` → `reviewLabel`                  |
...
Read the review markdown at `reviewPath`. Parse out:
...
### Step 2: Build the `create_review` call
...
create_review({
  ...
  reviewLabel:   "<reviewLabel>",           // optional — the review-kind label
...
The review is created as a **DRAFT**; `create_review` does not publish it.

### Step 3: Call the tool and capture the result

Call `create_review(...)`. On success it returns:

```json
{ "ok": true, "id": "review-…", "slug": "<slug>", "url": "https://app.broadbanner.com/app/reviews/<slug>" }
```
...
- **The portal URL** — `url` from the response (`https://app.broadbanner.com/app/reviews/<slug>`)
...
- **Validation fails (missing title/body):** report exactly what's missing; do not publish
  a partial review.
- **`create_review` authorization error:** the session isn't entitled — stop and report.
  No Pages/git fallback exists in this plugin.
- **`create_review` request-shape error:** fail loud; fix the offending argument (usually a
  bad `seriesId`/`showId` or an empty required field) and re-call. Don't retry blindly.
- **`create_review` transient error:** retry up to 3× with backoff, then report the failure
  — the review markdown is still on disk at `reviewPath`, so the user can re-run this skill.
...
instead: `create_review` stores the review as a draft the creator edits and publishes from
