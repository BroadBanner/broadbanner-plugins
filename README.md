...
- `broadbanner-live-production` — Creator+ live-production automation for BroadBanner creators. Schedule Substack and Restream live streams for the series you host, and publish clips to TikTok, YouTube, and Facebook via the Restream clips UI. Maps to the creator_workspace entitlement (Creator+).
- `broadbanner-post-production` — Production+ post-production automation for BroadBanner creators. Turn a finished Substack live draft into a publication-ready review: file the draft under its series section, download the transcript, correct it against the LIVE roster of who was in the room, generate the review with the show's server-sourced editorial config, and push it as an editable DRAFT into the member portal (app.broadbanner.com). The operator supplies only the Substack draft URL and the series name — everything else is derived through the BroadBanner MCP connector. Maps to the post_production_distribution add-on (Production+).
- `broadbanner-pre-production` — Pre-Production Assistant (Production+ add-on) for BroadBanner creators. Auto-schedules your shows on Substack and Restream on a recurring basis across every brand you host — the unattended automation of the manual schedule-live skills. Maps to the pre_production_assistant entitlement (cap scheduling:auto). Installs its recurring tasks through the broadbanner-scheduled-tasks engine.
...
- `broadbanner-social-distribution` — Social posting toolkit for BroadBanner creators (Core tier / banner_blast). Post text, image, and video notes to Substack/Bluesky/Threads and release queued Substack notes and clips. Runs on a single connected Chrome profile. Live-stream scheduling moved to the broadbanner-live-production plugin.
