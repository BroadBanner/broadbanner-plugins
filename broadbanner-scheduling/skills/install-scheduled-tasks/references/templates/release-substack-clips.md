...
1. Calls the `list_pending_clips` tool. **If nothing is pending, it exits immediately** with a one-line "nothing pending" report — the common case, not an error. It does not open Substack.
2. Otherwise (oldest first, up to 2 per run): calls `get_creator_context` for the Substack handle, fetches each clip in-page from `media.broadbanner.com` and injects it into the Substack Notes composer (the `substack-video-note` technique — no download-to-disk, no 10 MB cap), posts it, then calls `mark_substack_posted` to flip that tracker's substack slot `pending → posted` (or `failed`). Any remainder goes out on the next run.
